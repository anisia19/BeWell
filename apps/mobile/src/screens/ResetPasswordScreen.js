...
export default function ResetPasswordScreen({ onBack, userEmail }) {
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      if (data.success) {
        Alert.alert('Succes', 'Parola a fost schimbată! Te poți autentifica acum.', [
          { text: 'OK', onPress: onBack }
        ]);
      } else {
        Alert.alert('Eroare', data.error || 'Nu s-a putut schimba parola.');
      }
    } catch (error) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.inner} keyboardShouldPersistTaps="handled">
      <View style={styles.logoContainer}>
        ...
        <Text style={styles.subtitle}>Introdu parola inițială și setează una nouă</Text>
      </View>
      ...
      <View style={styles.form}>
        ...
        <TextInput
          style={styles.input}
          placeholder="Parola inițială dată de admin"
          placeholderTextColor="#4B5563"
          value={oldPassword}
